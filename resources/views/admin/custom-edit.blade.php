...
<div class="container">
    <div class="row">
        <div class="col-12">
            ...
            <form action="/customm/update/{{$as->custom_id}}" method="POST">
                @csrf
                @method('PATCH')
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control @error('nama') is-invalid @enderror" id="nama" placeholder="Masukkan total harga pesanan" name="nama" value="{{ str_replace(array('[','"',']'),'',$produk->pluck('nama_kategori'))}}" required readonly>
                    @error('nama')
                    <div class="alert alert-danger">{{ $message }}</div>
                    @enderror
                </div>
